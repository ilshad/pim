...
(defun listing-page (label-function
		     &key
		       (sources-key :sources)
		       (listing-key :listing)
		       (page-size *default-listing-page-size*))
  #'(lambda (state)
      (let ((sources (cdr (assoc sources-key state))))
	(if sources
	    (let* ((page? (> (length sources) page-size))
		   (page (if page? (subseq sources 0 page-size) sources)))
	      (append
	       (list
		(cons listing-key
		      (append
		       (loop for source in page
			     ...
			     collect (list :label (funcall label-function source)
					   :index index
					   :source source))
		       (if page?
			   '((:label "Cancel" :index "C")
			     (:label "...more" :index " "))
			   '((:label "Done" :index " ")))))
		(cons sources-key (when page? (subseq sources page-size))))
	       state))
	    state))))
